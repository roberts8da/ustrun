...
DEFAULT_VERSION="1.13.14"
latest_version=""

echo "[INFO] 获取 sing-box 最新版本..."
if version_json=$(curl -s --fail "https://api.github.com/repos/SagerNet/sing-box/releases" 2>/dev/null); then
    latest_version=$(echo "$version_json" | jq -r '[.[] | select(.prerelease==false)][0].tag_name | sub("^v"; "")' 2>/dev/null) || true
fi

if [ -z "$latest_version" ]; then
    echo "[WARN] 无法获取最新版本，回退到 v${DEFAULT_VERSION}"
